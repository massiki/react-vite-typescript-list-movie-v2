...
const TopRatedSeriesTv = () => {
  const { apiTopRatedSeriesTv, apiGenresSeriesTv } = useApiSeriesTv()
  const [topRatedSeriesTv, setTopRatedSeriesTv] = useState<seriesTvType[]>([])
  ...
  const {
    ...
    isLoading,
    searchSeriesTv,
    setIsLoading } = useSeriesTvSearchAndPagination({ defaultSeriesTv: topRatedSeriesTv })

  useEffect(() => {
    const load = async () => {
      setIsLoading(true)
      const resSereiesTv = await apiTopRatedSeriesTv(currentPage).then((res) => { return res })
      const resGenresSeriesTv = await apiGenresSeriesTv().then((res) => { return res })
      setTopRatedSeriesTv(resSereiesTv)
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="rounded-full bg-secondary px-3 py-1 text-sm font-medium text-secondary-foreground">
              {topRatedSeriesTv.length} Series TV
            </span>
          </div>
